...
variables<- c("SES_LEVEL_10","TP_COR_RACA",years_ses10)
f1=as.formula(paste("y~", paste(variables, collapse="+")))

variables1<- c("SES_LEVEL_25","TP_COR_RACA",years_ses25)
f2 <- as.formula(paste("y ~ ", paste(variables1, collapse = " + ")))

variables <- c("TP_COR_RACA", "INSE", years_cor_raca)
...
linear_regression <- function(dt, f) {
  lm_model <- lm(f, data = dt) # replace Score, A, B, C with your own variable names
